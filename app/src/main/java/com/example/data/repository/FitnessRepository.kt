...
class FitnessRepository(private val db: AppDatabase) {
    ...
    fun getUserProfile(email: String): Flow<UserProfile?> = db.userProfileDao().getUserProfile(email)
    ...
    fun getSessionsForUser(email: String): Flow<List<WorkoutSession>> = db.workoutSessionDao().getAllSessions(email)
    fun getWeeklyLogsForUser(email: String): Flow<List<HealthTrackerLog>> = db.healthTrackerLogDao().getWeeklyLogs(email)
    fun getAllLogsForUser(email: String): Flow<List<HealthTrackerLog>> = db.healthTrackerLogDao().getAllLogsForUser(email)

    suspend fun getUserProfileDirect(email: String): UserProfile? = db.userProfileDao().getUserProfileDirect(email)
    ...
    suspend fun deleteUserProfile(email: String) {
        db.userProfileDao().deleteUserProfile(email)
    }
    ...
    suspend fun saveWorkoutSession(session: WorkoutSession): Long {
        val rowId = db.workoutSessionDao().insertSession(session)
        val updatedSession = session.copy(id = rowId)
        return rowId
    }
    ...
    fun getHealthLogForDate(dateKey: String, email: String): Flow<HealthTrackerLog?> {
        return db.healthTrackerLogDao().getLogForDate(dateKey, email)
    }
    ...
    suspend fun incrementSteps(email: String, amount: Int) {
        stepMutex.withLock {
            withContext(Dispatchers.IO) {
                val today = getTodayDateKey()
                val current = db.healthTrackerLogDao().getLogForDateDirect(today, email) ?: HealthTrackerLog(
                    id = "$today:$email",
                    dateKey = today,
                    userEmail = email
                )
                ...
                db.healthTrackerLogDao().insertHealthTrackerLog(updatedLog)
            }
        }
    }

    suspend fun addWaterIntake(email: String, amountMl: Int) {
        val today = getTodayDateKey()
        val current = db.healthTrackerLogDao().getLogForDateDirect(today, email) ?: HealthTrackerLog(
            id = "$today:$email",
            dateKey = today,
        ...
        val updatedLog = current.copy(waterIntakeMl = current.waterIntakeMl + amountMl)
        db.healthTrackerLogDao().insertHealthTrackerLog(updatedLog)
    }

    suspend fun addActiveCalories(email: String, calories: Double) {
        val today = getTodayDateKey()
        val current = db.healthTrackerLogDao().getLogForDateDirect(today, email) ?: HealthTrackerLog(
            id = "$today:$email",
            dateKey = today,
        ...
        val updatedLog = current.copy(activeCaloriesBurned = current.activeCaloriesBurned + calories)
        db.healthTrackerLogDao().insertHealthTrackerLog(updatedLog)
    }

    // --- SEEDING INITIAL EXERCISES ---
    suspend fun seedInitialExercisesIfEmpty() {
        val existing = db.exerciseDao().getAllExercisesDirect()
    ...
    suspend fun generateWorkoutSchedule(profile: UserProfile) {
        ...
        seedInitialExercisesIfEmpty()

        val equip = profile.equipmentInventory
        val userGender = profile.gender
        val level = profile.experienceLevel

        // Get matching exercises
        val allAvailable = db.exerciseDao().getAllExercisesDirect()
        ...
        fun getExsByMuscleAndLevel(muscleGroup: String, count: Int): List<String> {
            ...
            val preferred = muscleExs.filter { it.difficultyLevel.equals(level, ignoreCase = true) }
            val finalPool = if (preferred.isNotEmpty()) {
                preferred
            } else {
                val fallbackLevel = when (level) {
                    "Advanced" -> "Intermediate"
                    "Intermediate" -> "Beginner"
                    else -> "Beginner"
                }
                val secondary = muscleExs.filter { it.difficultyLevel.equals(fallbackLevel, ignoreCase = true) }
                if (secondary.isNotEmpty()) secondary else muscleExs
            }
